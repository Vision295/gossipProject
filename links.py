...
def create_topology(project_name="link test"):
    """
    ...
    # Create nodes
    pc1 = Node(name="PC1", project_id=project.project_id, connector=server,
               template=VPCS_TEMPLATE, x=0, y=0)
    pc2 = Node(name="PC2", project_id=project.project_id, connector=server,
               template=VPCS_TEMPLATE, x=200, y=0)
    switch = Node(name="Switch", project_id=project.project_id, connector=server,
                  template=SWITCH_TEMPLATE, x=100, y=-100)

    for node in [pc1, pc2, switch]:
        node.create()
...
def get_project_info(project):
    ...
    print("\nLinks:")
    for l in links:
        print(f" - Link ID: {l.link_id}, Nodes: {[(n['node_id']) for n in l.nodes]}")
...
if __name__ == "__main__":
    ...
    # 2️⃣ Retrieve project info
    nodes, links = get_project_info(proj)

    # 3️⃣ Example: modify a link (e.g. suspend the first one)
    modify_link(
        proj,
